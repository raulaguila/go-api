...
func (s *DepartmentRepositoryMock) CreateDepartment(ctx context.Context, department *domain.Department) error {
	ret := s.Called(ctx, department)
	return ret.Error(0)
}

func (s *DepartmentRepositoryMock) UpdateDepartment(ctx context.Context, department *domain.Department) error {
	ret := s.Called(ctx, department)
	return ret.Error(0)
}
